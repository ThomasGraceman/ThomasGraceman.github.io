...
<div id="tree-viz-root" style="margin: 30px 0;"></div>
...
<div class="viz-card">
  ...
  <div style="margin-top: 10px; font-size: 12px; color: #aaa; text-align: center; border-top: 1px solid #eee; padding-top: 8px;">
    Click a level button to highlight the partition. Hover over a node to see cluster details. Edge lengths are proportional to $2^i$.
  </div>
</div>
...
(function() {
  ...
  // Shadow filter for highlighted nodes
  defs.append('filter')
    .attr('id', 'node-shadow')
    .append('feDropShadow')
    .attr('dx', 0)
  ...
  // Override y positions to align with levels
  root.descendants().forEach(d => {
    d.y = levelY[d.data.level];
  });

  const g = svg.append('g')
    .attr('transform', `translate(${margin.left},${margin.top})`);

  // Level background markers
  const levelMarkers = g.append('g').attr('class', 'level-markers');
  [0, 1, 2].forEach(l => {
    ...
    levelMarkers.append('text')
      .attr('x', innerW + 20)
      .attr('y', levelY[l] + 4)
      .attr('class', 'level-marker')
      .text(`D${l}`);
  });

  // Edge length labels
  const edgeLabels = [
    { from: levelY[2], to: levelY[1], label: '2² = 4', x: innerW * 0.15 },
    { from: levelY[1], to: levelY[0], label: '2¹ = 2', x: innerW * 0.15 }
  ];
  edgeLabels.forEach(({ from, to, label, x }) => {
    g.append('text')
      .attr('x', x)
      .attr('y', (from + to) / 2 + 3)
      .attr('class', 'edge-length-label')
      .text(label);
  });
  ...
  // Cluster color scales
  const colorsForLevel = {
  ...
  // Precompute cluster assignment: for each node and each level, which cluster does it belong to?
  const clusterMap = {};
  root.descendants().forEach(d => {
    clusterMap[d.data.id] = {};
    [0, 1, 2].forEach(l => {
      let cur = d;
      while (cur && cur.data.level !== l) cur = cur.parent;
      clusterMap[d.data.id][l] = cur ? cur.data.id : null;
    });
  });

  // Helper: get cluster id for a node at selected level
  ...
  node.append('circle')
    ...
    .attr('r', d => d.data.level === 2 ? 14 : d.data.level === 1 ? 9 : 6)
    .attr('fill', '#f5f5f5')
    .attr('stroke', '#ccc')
    .attr('stroke-width', 2);
  ...
  node.append('text')
    ...
    .attr('x', d => d.data.level === 0 ? 10 : 0)
    .attr('y', d => d.data.level === 0 ? 4 : -(d.data.level === 2 ? 20 : 14))
    .attr('text-anchor', d => d.data.level === 0 ? 'start' : 'middle')
    .text(d => d.data.name);

  // Level annotation for internal nodes
  node.filter(d => d.data.level > 0)
    .append('text')
    .attr('class', 'node-label-level')
    .attr('y', d => d.data.level === 2 ? -34 : -26)
    .attr('text-anchor', 'middle')
    .text(d => `(radius ${d.data.radius})`);

  // Tooltip
  const tooltip = d3.select('#tree-viz-root')
    .append('div')
  ...
  node.on('mouseenter', function(ev, d) {
    tooltip
      .style('opacity', 1)
      .html(`<b>${d.data.name}</b><br>Level: D${d.data.level}<br>Radius: ${d.data.radius}`)
      .style('left', (ev.offsetX + 15) + 'px')
  ...
  function updateViz(level) {
    ...
    d3.select('#partition-label').text(partitionLabels[level]);

    // Update edges: color by whether they belong to highlighted clusters
    link.transition().duration(400)
      .attr('stroke', d => {
        const srcLevel = d.source.data.level;
        const tgtLevel = d.target.data.level;
        if (srcLevel >= level && tgtLevel <= srcLevel) return '#888';
        return '#ddd';
      })
      .attr('stroke-width', d => {
        const srcLevel = d.source.data.level;
        return srcLevel >= level ? 2.5 : 1;
      })
      .attr('stroke-opacity', d => {
        const srcLevel = d.source.data.level;
        if (srcLevel >= level) return 0.8;
        return 0.3;
      });
    ...
    node.select('circle')
      ...
      .attr('r', d => {
        ...
        if (d.data.level === 2) return 14;
        if (d.data.level === 1) return 9;
        return 6;
      })
      .attr('fill', d => {
        if (d.data.level === level) {
          const colorScale = colorsForLevel[level];
          const cid = getClusterId(d, level);
          return colorScale(cid);
        }
        if (d.data.level > level) return '#f9f9f9';
        // Below the selected level: color by parent cluster
        const colorScale = colorsForLevel[level];
        const cid = getClusterId(d, level);
        const baseColor = d3.color(colorScale(cid));
        if (baseColor) {
      ...
      .attr('stroke', d => {
        if (d.data.level === level) {
          const colorScale = colorsForLevel[level];
          const cid = getClusterId(d, level);
          return d3.color(colorScale(cid)).darker(0.5);
        }
        if (d.data.level > level) return '#d0d0d0';
        return '#ccc';
      })
      .attr('stroke-width', d => {
        if (d.data.level === level) return 3;
        return 1.5;
      })
      .attr('filter', d => d.data.level === level ? 'url(#node-shadow)' : null);

    node.style('opacity', d => {
      if (d.data.level >= level) return 1;
      return 0.55;
    });

    // Highlight cluster boundaries: show rounded rect behind clusters at selected level
    // (skip for brevity — the color scheme already conveys this)
  }
  ...
  window.addEventListener('resize', () => {
    ...
    resizeTimer = setTimeout(() => {
      ...
      if (w > 0) {
        d3.select(container).select('svg').attr('viewBox', `0 0 ${totalWidth} ${totalHeight}`);
      }
    }, 200);
  });
})();
