...
Also note that we have $d^* = \mathcal{O}(\frac{d}{\varepsilon})$. To see that we have $\Omega(\varepsilon n)$, so $\frac{\sum_{u \in G}d_u}{n} = d \geq \rho d^* = \varepsilon n \frac{d^*}{n}$, then $\frac{d}{\varepsilon}$, so $d^* = \mathcal{O}(\frac{d}{\varepsilon})$. And if we scale $\varepsilon$ properly, with very high probability we have that only $\varepsilon \frac{n}{4}$ vertices have higher degree than $d^*$ (with high probability).
...
Hence we run the algorithm for $\mathcal{O}(r)$ edges and each time in each iteration we do this $\mathcal{O}({\frac{d}{\varepsilon^2}})$ times, $O(r) \cdot \frac{1}{n} \sum_{u \in V} d_u \log(W d^*) = O(dr \log(W d^*)) = O\left(d\varepsilon^{-2} \log \frac{d}{\varepsilon}\right)$.
...
Here comes another interesting technique: namely, suppose that you can compute the algorithm for some sort of inputs, then turn any other input to the desired input such that you can have the result back with an error of at most like something $\mathcal{O}(\varepsilon)$ or something along those lines...
